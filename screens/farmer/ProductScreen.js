...
const ProductScreen = () => {
          ...
          <View style={styles.popupContainer}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer}>
              ...
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailValue}>{selectedProduct.attributes.crop_name}</Text>
              </View>
              <View style={styles.detailItem}>
                <Text style={styles.detailLabel}>Price:</Text>
                <Text style={styles.detailValue}>{selectedProduct.attributes.price}</Text>
              </View>
              <View style={styles.detailItem}>
                <Text style={styles.detailLabel}>Product Quantity:</Text>
                <Text style={styles.detailValue}>{selectedProduct.attributes.quantity}</Text>
              </View>
              <View style={styles.detailItem}>
                <Text style={styles.detailLabel}>Description:</Text>
                <Text style={styles.detailValue}>{selectedProduct.attributes.description}</Text>
              </View>
  ...
  <Icon name="edit" size={20} color="#528508" />
                ...
                <TouchableOpacity onPress={() => deleteProduct(selectedProduct.id)}>
  ...
  const updateProduct = async () => {
    try {
      ...
      const response = await fetch(`${url}/api/products/${selectedProduct.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        Alert.alert('Update Successful', 'Are you sure you want to update this product?', [
          {
            text: 'Cancel',
            style: 'cancel',
          },
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.productItem} onPress={() => handleProductClick(item)}>
      <Text style={styles.productType}>{item.attributes.crop_name}</Text>
      <Text style={styles.productVariety}>price {item.attributes.price}</Text>
      <Text style={styles.productVariety}>quantity {item.attributes.quantity}</Text>
    </TouchableOpacity>
  );
